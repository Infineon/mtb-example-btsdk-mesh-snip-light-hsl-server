...
#define MESH_LIGHT_LIGHTNESS_DEFAULT            1
...
#if NUM_HSL_SERVERS > 1
wiced_bt_mesh_core_config_model_t   mesh_element4_models[] =
{
    WICED_BT_MESH_MODEL_LIGHT_HSL_SERVER,
};

wiced_bt_mesh_core_config_model_t   mesh_element5_models[] =
{
    WICED_BT_MESH_MODEL_LIGHT_HSL_HUE_SERVER,
};

wiced_bt_mesh_core_config_model_t   mesh_element6_models[] =
{
    WICED_BT_MESH_MODEL_LIGHT_HSL_SATURATION_SERVER,
};
#endif
...
wiced_bt_mesh_core_config_element_t mesh_elements[] =
{
    ...
    },
    {
        ...
        .onpowerup_state = WICED_BT_MESH_ON_POWER_UP_STATE_RESTORE,     // Default element behavior on power up
        .default_level = 0,                                             // Default value of the variable controlled on this element (for example power, lightness, temperature, hue...)
        .range_min = 0,                                                 // Minimum value of the variable controlled on this element (for example power, lightness, temperature, hue...)
        .range_max = 0xffff,                                            // Maximum value of the variable controlled on this element (for example power, lightness, temperature, hue...)
        .move_rollover = 0,                                             // If true when level gets to range_max during move operation, it switches to min, otherwise move stops.
        .properties_num = 0,                                            // Number of properties in the array models
        .properties = NULL,                                             // Array of properties in the element.
        .sensors_num = 0,                                               // Number of sensors in the sensor array
        .sensors = NULL,                                                // Array of sensors of that element
        .models_num = (sizeof(mesh_element5_models) / sizeof(wiced_bt_mesh_core_config_model_t)),  // Number of models in the array models
        .models = mesh_element5_models,                                 // Array of models located in that element. Model data is defined by structure wiced_bt_mesh_core_config_model_t
    },
    {
        .location = MESH_ELEM_LOC_MAIN,                                 // location description as defined in the GATT Bluetooth Namespace Descriptors section of the Bluetooth SIG Assigned Numbers
        .default_transition_time = MESH_DEFAULT_TRANSITION_TIME_IN_MS,  // Default transition time for models of the element in milliseconds
        ...
        .sensors_num = 0,                                               // Number of sensors in the sensor array
        .sensors = NULL,                                                // Array of sensors of that element
        .models_num = (sizeof(mesh_element6_models) / sizeof(wiced_bt_mesh_core_config_model_t)),  // Number of models in the array models
        .models = mesh_element6_models,                                 // Array of models located in that element. Model data is defined by structure wiced_bt_mesh_core_config_model_t
    },
#endif
};
...
#if NUM_HSL_SERVERS > 1
    wiced_bt_mesh_model_light_hsl_server_init(MESH_LIGHT_HSL_SERVER_ELEMENT_INDEX + 3, mesh_app_message_handler, is_provisioned);
#endif
}
...
uint32_t mesh_app_proc_rx_cmd(uint16_t opcode, uint8_t *p_data, uint32_t length)
{
    ...
    case HCI_CONTROL_MESH_COMMAND_LIGHT_HSL_SET:
        ...
        mesh_light_hsl_server_status_changed(element_idx, p_data, length);
        break;
...
/*
 * Send Light HSL Status event
 */
void mesh_light_hsl_server_status_changed(uint8_t element_idx, uint8_t *p_data, uint32_t length)
{
    wiced_bt_mesh_light_hsl_status_data_t data;

    STREAM_TO_UINT16(data.present.lightness, p_data);
    STREAM_TO_UINT16(data.present.hue, p_data);
    STREAM_TO_UINT16(data.present.saturation, p_data);
    data.remaining_time = 0;

    wiced_bt_mesh_model_light_hsl_server_state_changed(element_idx, &data);
}
